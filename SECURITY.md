...
See [ARCHITECTURE.md](ARCHITECTURE.md) for the detailed enforcement model.
...
Any module that allows configurable SSL verification (e.g. SIEM/syslog TLS in
`daemon/security/siem_integration.py`) requires quarterly review to confirm
production deployments use `CERT_REQUIRED`.
...
|---------|----------|---------|
| Security Scan (LLM-assisted) | `security_scan` / `daemon/security/code_advisor.py` | Pre-commit, PR |
| Credential Scanner | `.githooks/pre-commit` | Every commit |
...
- [ARCHITECTURE.md](ARCHITECTURE.md) - System architecture and enforcement model
